...
class CoinWorkerTest {
    ...
    @Test
    fun `runOnce skips entry when position exists and no exit signal`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
            coEvery { rateLimiter.acquire() } just Runs
            every { trader.getCurrentPosition(contract) } returns
                Position(
                    contract,
                    1L,
                    "50000",
    ...
    @Test
    fun `runOnce closes position when exit signal triggers`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
            ...
            every { trader.closePosition(contract) } returns
                OrderResult(
                    100L,
                    contract,
                    0L,
    ...
    @Test
    fun `runOnce does not close when exit signal is None`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
    ...
    @Test
    fun `runOnce opens long when long signal and no position`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
    ...
    @Test
    fun `runOnce opens short when short signal and no position`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
            ...
            every { trader.openShort(contract, 1, 5) } returns
                OrderResult(
                    2L,
                    contract,
                    -1L,
    ...
    @Test
    fun `runOnce does nothing on None signal`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
    ...
    @Test
    fun `runOnce swallows exception from trader to keep worker alive`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
    ...
    @Test
    fun `updateCandles uses delta fetch after first call`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
    ...
    @Test
    fun `full cycle - long entry then exit closes position`() =
        runTest {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
            ...
            val fallingCandles = fallingPriceCandles(20)
            every { marketData.getCandles(contract, Interval.MIN_1, 100, null, null) } returns fallingCandles
            every { trader.openLong(contract, 1, 5) } returns
                OrderResult(
                    1L,
                    contract,
                    1L,
    ...
    @Test
    fun `run exits gracefully on cancellation`() =
        runBlocking {
            val trader = mockk<FuturesTrader>()
            val marketData = mockk<MarketDataService>()
            val rateLimiter = mockk<RateLimiter>()
    ...
    private fun flatPriceCandles(
        count: Int,
        price: Double,
    ): List<Candle> = (0 until count).map { candle(1700000000L + it * 60, price) }

    private fun fallingPriceCandles(count: Int): List<Candle> = (0 until count).map { candle(1700000000L + it * 60, 25.0 - it * 0.5) }

    private fun risingPriceCandles(count: Int): List<Candle> = (0 until count).map { candle(1700000000L + it * 60, 100.0 + it.toDouble()) }
}
