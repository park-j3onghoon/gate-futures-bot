...
class WorkerOrchestratorTest {
    ...
    fun `createWorker applies BotProperties`() {
        val orchestrator = WorkerOrchestrator(
    ...
    @Test
    fun `runAll launches a worker for each contract`() = runBlocking {
    ...
    @Test
    fun `runAll isolates worker failure via supervisorScope`() = runBlocking {
        val trader = mockk<FuturesTrader>()
        val marketData = mockk<MarketDataService>()
        val rateLimiter = mockk<RateLimiter>()
