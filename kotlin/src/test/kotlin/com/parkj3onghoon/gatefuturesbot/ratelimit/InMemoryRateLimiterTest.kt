...
class InMemoryRateLimiterTest {
    @Test
    fun `acquire succeeds immediately when tokens available`() =
        runBlocking {
    ...
    @Test
    fun `acquire waits when tokens exhausted and refills over time`() =
        runBlocking {
    ...
    @Test
    fun `acquire uses injected clock to compute refill`() =
        runBlocking {
            var fakeNanos = 0L
            val limiter =
                InMemoryRateLimiter(
